...
class base(object):
    ...
    def check(self):
        ...
        if 'PICKOBSDISPER' not in PAR:
            setattr(PAR, 'PICKOBSDISPER', 'true')
    ...
    def prepare_eval_grad(self, path='.'):
        ...
        for filename in solver.data_filenames:
            ...
            for ii in range(nn):
                if(np.max(np.abs(obs[ii].data)))>0:
                    continue
                else:
                    syn[ii].data[:]=0
            ...
            self.writer(syn, path+'/'+'traces/syn',filename+'_proc') 
            ...
            self.writer(obs, path+'/'+'traces/obs',filename+'_proc')
    ...
    def apply_window(self, syn, windows):
        ...
        for ii in range(nn):
            syn[ii].data = np.multiply(syn[ii].data, windows[ii].data)
    ...
    def write_residuals(self, path, syn, obs):
        ...
        ##################################
        #2018.7.17 bug fixed by zhaolun
            #temp = np.loadtxt(filename)
            #residuals.extend([temp.tolist()])
        ##################################
    ...
    def apply_filter(self, traces):
        if not PAR.FILTER:
            ...
            for tr in traces:
                ...
                tr.filter('bandpass', freqmin=PAR.FREQMIN, freqmax=PAR.FREQMAX,corners=2,zerophase=True)
            ...
            for tr in traces:
                ...
                tr.detrend('linear')
                tr.taper(0.05, type='hann')
                tr.filter('lowpass', freq=PAR.FREQ,corners=2,zerophase=True)
            ...
            for tr in traces:
                ...
                tr.detrend('linear')
                tr.taper(0.05, type='hann')
                tr.filter('highpass', freq=PAR.FREQ,corners=2,zerophase=True)
    ...
    def apply_normalize(self, traces):
        ...
        if 'NormalizeTracesL1' in PAR.NORMALIZE:
            ...
            for tr in traces:
                ...
                if w > 0:
                    tr.data /= w
        elif 'NormalizeTracesMAX' in PAR.NORMALIZE:
            for tr in traces:
