...
class specfem2d(custom_import('solver', 'base')):
    ...
    def check_solver_parameter_files(self):
        ...
        if nt != PAR.NT:
            if self.taskid == 0: print "WARNING: NSTEP != PAR.NT"
            setpar('NSTEP', PAR.NT)
    ...
    def smooth(self, input_path='', output_path='', parameters=[], span=0.):
        ...
        """
        if not exists(PATH.SPECFEM_BIN+'/'+'xsmooth_sem'):
            smooth_legacy(input_path, output_path, parameters, span)
        else:
            ...
            unix.cd(self.cwd)
            print ' cwd', self.cwd
            src = glob(join(self.cwd, 'DATA/pro*_x.bin'))
            dst = input_path
            unix.cp(src, dst)
            src = glob(join(self.cwd, 'DATA/pro*_z.bin'))
            unix.cp(src, dst)
            src = glob(join(self.cwd, 'DATA/pro*_NSPEC_ibool.bin'))
            if not exists(src):
                raise Exception
            unix.cp(src, dst)
            src = glob(join(self.cwd, 'DATA/pro*_jacobian.bin'))
            if not exists(src):
                raise Exception
            unix.cp(src, dst)
            for name in parameters or self.parameters:
                print ' smoothing', name
                #print(PATH.SPECFEM_BIN +'/'+ 'xsmooth_sem '
                #    + str(span) + ' '
                #    + str(span) + ' '
                #    + name + '_kernel' + ' '
                #    + input_path + '/ '
